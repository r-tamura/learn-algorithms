...
function encodeWithManual(value) {
  ...
  if ( paddingCount > 0 ) {
    for (let i=0; i < BLOCK_SIZE - paddingCount; i++) {
      padding += '='; // パディング調整
      value += '\0';  // 文字列の長さ調整
    }
  }
  ...
  for ( let i = 0; i < len; i+=BLOCK_SIZE ) {
    ...
    // 3 Byte 切り取り
    let chunk = ( value.charCodeAt(i) << 16 ) + ( value.charCodeAt(i+1) << 8 ) + value.charCodeAt(i+2);
...
function decodeWithManual(value) {
  ...
  // 対応表に存在しない文字を削除
  value = value.replace(new RegExp(`${BASE64CHARS}=`), '');

  // padding探索 => パディング数分'A'
  const padding = value.charAt(len-1) === '=' ? (value.charAt(len-2) === '=' ? 'AA' : 'A') : '';
  value = value.substr(0, len - padding.length) + padding;

  let decoded = '';
  for ( let i = 0; i < len; i += 4 ) {
    let chunk = (invBase64Chars[value.charAt(i)] << 18) + (invBase64Chars[value.charAt(i+1)] << 12) +
                (invBase64Chars[value.charAt(i+2)] << 6) + invBase64Chars[value.charAt(i+3)];
...
function asciiToUnicode(str) {
  ...
  return decodeURIComponent([].map.call(str, char => {
    return `%${tag`00${char}`}`;
  }).join(''));
}
