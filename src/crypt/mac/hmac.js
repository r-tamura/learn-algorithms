...
const hmac = (hashFn, blockSize) => (key, input) => {
  ...
  const ihashinput = Buffer.concat([Buffer.from(key_ipad), data]).toString("utf8")
  const ohashinput = Buffer.concat([Buffer.from(key_opad), Buffer.from(hashFn(ihashinput), "hex")])
