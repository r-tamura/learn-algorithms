import hash from "./hash";
import { Buffer } from "node:buffer";

const K = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
  0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
  0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
  0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
  0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
  0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
];

const H = [
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c,
  0x1f83d9ab, 0x5be0cd19,
];

const rotRight = (r: number, x: number): number => {
  const rotation = r % 32;
  return (x << (32 - rotation)) | (x >>> rotation);
};

const sigma0 = (x: number): number =>
  rotRight(7, x) ^ rotRight(18, x) ^ (x >>> 3);
const sigma1 = (x: number): number =>
  rotRight(17, x) ^ rotRight(19, x) ^ (x >>> 10);
const ch = (x: number, y: number, z: number): number => (x & y) ^ (~x & z);
const maj = (x: number, y: number, z: number): number =>
  (x & y) ^ (x & z) ^ (y & z);
const sum0 = (x: number): number =>
  rotRight(2, x) ^ rotRight(13, x) ^ rotRight(22, x);
const sum1 = (x: number): number =>
  rotRight(6, x) ^ rotRight(11, x) ^ rotRight(25, x);

const hashBlock = (prevHashState: number[], block: Buffer): number[] => {
  const w = new Uint32Array(64);
  for (let i = 0; i < 16; i += 1) {
    w[i] = block.readUInt32BE(i * 4);
  }

  for (let t = 16; t < 64; t += 1) {
    w[t] = (sigma1(w[t - 2]) + w[t - 7] + sigma0(w[t - 15]) + w[t - 16]) >>> 0;
  }

  let [a, b, c, d, e, f, g, h] = prevHashState;
  for (let t = 0; t < 64; t += 1) {
    const tmp1 = h + sum1(e) + ch(e, f, g) + K[t] + w[t];
    const tmp2 = sum0(a) + maj(a, b, c);
    h = g;
    g = f;
    f = e;
    e = (d + tmp1) >>> 0;
    d = c;
    c = b;
    b = a;
    a = (tmp1 + tmp2) >>> 0;
  }

  return [
    (prevHashState[0] + a) >>> 0,
    (prevHashState[1] + b) >>> 0,
    (prevHashState[2] + c) >>> 0,
    (prevHashState[3] + d) >>> 0,
    (prevHashState[4] + e) >>> 0,
    (prevHashState[5] + f) >>> 0,
    (prevHashState[6] + g) >>> 0,
    (prevHashState[7] + h) >>> 0,
  ];
};

const sha256 = hash(hashBlock, H);

export default sha256;
